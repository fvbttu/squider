function [pset, opstr] = squidfit(ydata, p1, y1, fixvals, fyi, usefun, varargin)
...
% parameters to hold fixed (empty if none are); fyi specifies which
% compartments to fit to using squid2sol specs (by default, all detected,
% with deaths and recovered included separately depending on the number of
% columns in ydata). Arg usefun specifies which fitting function to use
% (0 = lsqcurvefit, default; 1 = fminsearch); subsequent arguments are sent
% to the fitting routine.

[m, n] = size(ydata);
if nargin < 5 || isempty(fyi)
    if n == 3
        retvals = {4:6,5,6};
    elseif n == 2
        retvals = {4:6,6};
