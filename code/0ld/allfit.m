function Tout = allfit(T, nq, n1, ng, p1)
% Tout = allfit(T, nq, n1, ng, p1)
%
...
% (default = 101 days), ng = date of change in death rate (default = 120),
% p1 = init guess for parameters; if a duple, this is just a guess for the
% 1st q pulse, and default vals are used for the other params.
...
tmplate = NaN(1,10 + 2*nq); jj = [1:6,9:11,length(tmplate)];   % template for coversion of param sets
tmplate(7) = ng; tmplate(8) = p1(6); tmplate(12:4:end) = -0.5;
tmplate(13:2:end) = (n2-n1)*(1:2:(2*(nq-1)))/(2*(nq-1)) + n1; tmplate(14:4:end) = q1(1);
for i = 1:length(cn)
    disp([num2str(i), ': ', T.State{cn{i}}]);
    qtmp = tmplate; qtmp(jj) = cp1{i};
    [cp2{i},co2{i}] = squidfit2d(cyy{i}, qtmp(1:end-1), qtmp(end), jj);
    ytmp = squid2sol2d(cp2{i}(1:end-1), n2, cp2{i}(end), {4:6,6});
    cr2{i} = r2tst('r2', cyy{i}, ytmp, length(cp2{i})-length(jj));
...
for i = 1:length(cn)
    disp([num2str(i), ': ', T.State{cn{i}}]);
    [cp3{i},co3{i}] = squidfit2d(cyy{i}, cp2{i}(1:end-1), cp2{i}(end));
    cy3{i} = squid2sol2d(cp3{i}(1:end-1), 730, cp3{i}(end));
...
elseif all(ll)
    ...
    while any(ll)
        for i = 1:length(kk)
            ...
            [~, jk] = min(tdist(jj,i)); tried(jj(jk),i) = true;
            disp(['Refitting ' cn{kk(i)} ' based on ' cn{jj(jk)}]);
            [ptmp, otmp] = squidfit2d(cyy{kk(i)}, cp3{jj(jk)}(1:end-1), cp3{jj(jk)}(end));
            ytmp = squid2sol2d(ptmp(1:end-1), 730, ptmp(end));
